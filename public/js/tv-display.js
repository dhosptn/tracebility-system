...
function updateStatusTimer() {
    ...
    // Stop updating if status is Finish
    if (state.currentStatus.toLowerCase() === "finish") return;
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success) {
                ...
                const finalDuration = data.final_duration;
                ...
                if (
                    ...
                    updateTimerStatus(newStatus, startTime, finalDuration);
                }
            }
        },
        error: function (err) {
...
window.updateTimerStatus = function (
    newStatus,
    startTime = null,
    finalDuration = null
) {
    if (
        ...
        if (newStatus.toLowerCase() === "finish") {
            if (finalDuration !== null) {
                $("#currentTimer").text(formatDurationHHMMSS(finalDuration));
            }
            // If finalDuration is null, we just keep whatever is currently on the timer
            state.statusStartTime = null;
            console.log("Timer STOPPED for status: Finish");
        } else if (startTime) {
            state.statusStartTime = new Date(startTime).getTime();
            console.log(
                "Timer SYNCED for status:",
                newStatus,
                "StartTime:",
                startTime
            );
            // We don't necessarily reset to 00:00:00 here, updateStatusTimer will fix it on next tick
        } else {
...
function checkProductionFinish(qtyOk, targetQty) {
    // If we don't have args, try to read from DOM or state?
    // Better to pass them in.
    if (qtyOk === undefined) qtyOk = state.previousData.qty_ok;
    ...
    const currentTimerLabel = $("#currentTimer")
        .parent()
        .find("div:first-child");
    const currentTimerContainer = $("#currentTimer").parent();

    if (qtyOk >= targetQty && targetQty > 0) {
        // FINISH STATE
        ...
        currentTimerContainer
            ...
            .addClass("from-green-600 to-green-700");

        // Note: The timer stopping is handled in updateStatusTimer and updateTimerStatus
        // based on state.currentStatus === 'Finish'
    } else {
        // NORMAL STATE
        if (currentTimerLabel.length && currentTimerLabel.text() === "FINISH") {
            ...
            currentTimerLabel
                ...
                .addClass("text-white");
            currentTimerContainer
                .removeClass("from-green-600 to-green-700")
...
function updateStatus(status) {
    ...
    if (s === "running") {
        ...
        html =
            '<div class="text-lg font-black text-white tracking-widest">FINISH</div>';
    } else if (s === "downtime") {
...
function updateTimeline(timeline) {
    ...
    for (let i = 0; i < labelCount; i++) {
        ...
        const labelDiv = $("<div></div>").addClass(
...
function fetchData() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            const progress =
                data.wo_qty > 0
                    ? ((data.qty_ok / data.wo_qty) * 100).toFixed(1)
                    : "0.0";
            ...
            if (data.current_status) {
                updateTimerStatus(
                    data.current_status,
                    data.current_status_start_time,
                    data.final_duration
                );
            }
            ...
            // Check Finish
            checkProductionFinish(data.qty_ok, data.wo_qty);

            // Auto Save
            if (
                data.qty_ok >= data.wo_qty &&
                !state.hasAutoSaved &&
                data.wo_qty > 0
            ) {
                autoSaveTransaction();
...
function checkMqttSignals() {
    ...
    // Check NG Signal
    $.get(
        `/production/production-monitoring/${state.monitoringId}/check-mqtt-ng-signal`,
        function (data) {
    ...
    $.get(
        `/production/production-monitoring/${state.monitoringId}/check-mqtt-downtime-signal`,
        function (data) {
            if (data.show) {
    ...
    // Check Status Signal
    $.get(
        `/production/production-monitoring/${state.monitoringId}/check-mqtt-status-signal`,
        function (data) {
            if (data.show && data.status) {
                console.log("MQTT Status Signal:", data);
                updateTimerStatus(data.status, null, data.final_duration);
            }
        }
    );
}
...
function openNgModal(data = {}) {
    ...
    if (data.auto_saved) {
        form.find('[name="notes"]').val(
            "Auto-saved from MQTT signal - Review and confirm"
        );
...
function openDowntimeModal(data = {}) {
    ...
    if (data.downtime_type)
        form.find('[name="downtime_type"]').val(data.downtime_type);
    if (data.downtime_reason)
        form.find('[name="downtime_reason"]').val(data.downtime_reason);
